...
from app.models.models import (
    CarbonProject, ProjectRating, FraudAlert, CarbonCredit,
...
async def _get_project_ids_with_positions(db: AsyncSession) -> set[int]:
    result = await db.execute(
        select(CarbonCredit.project_id)
        .join(PortfolioPosition, PortfolioPosition.credit_id == CarbonCredit.id)
        .distinct()
    )
...
def _generate_credits_forecast(project):
    ...
    project_duration = max(1, end_year - start_year)
    ...
    for year in range(vintage - min(3, vintage - start_year), vintage + 1):
        ...
        remaining_issued -= yp
        pm, rm = rng.choice([3, 6, 9, 12]), max(1, min(12, rng.choice([3, 6, 9, 12]) + rng.randint(-2, 4)))
        past.append({
            "vintage_year": year, "type": "emitido", "planned_quantity": yp,
            "planned_date": f"{year}-{pm:02d}", "realized_quantity": yr,
            "realized_date": f"{year}-{rm:02d}", "retired_quantity": yrt,
            "status": "concluído",
...
async def list_projects(
    ...
    if sort_field:
        field_map = {"name": CarbonProject.name, "project_type": CarbonProject.project_type,
    ...
    items = [_serialize_project(p, p.rating, len(p.fraud_alerts) if p.fraud_alerts else 0, p.id in ids_with_position) for p in projects]
    return {"items": items, "total": total, "page": page, "page_size": page_size, "total_pages": total_pages}


@router.get("/geo")
async def list_projects_geo(db: AsyncSession = Depends(get_db)):
    """Retorna dados geográficos otimizados para mapa."""
    result = await db.execute(
        select(
...
@router.get("/compare")
async def compare_projects(ids: str = Query(..., description="IDs separados por vírgula"), db: AsyncSession = Depends(get_db)):
    """Retorna N projetos para comparação side-by-side."""
    ...
    projects = result.scalars().unique().all()
    return [_serialize_project(p, p.rating, len(p.fraud_alerts) if p.fraud_alerts else 0) for p in projects]
...
async def get_project(project_id: int, db: AsyncSession = Depends(get_db)):
    ...
    return _serialize_project(project, project.rating, len(project.fraud_alerts) if project.fraud_alerts else 0, project_id in ids_pos, forecast)
...
@router.get("/{project_id}/rating", response_model=RatingResponse)
async def get_project_rating(project_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(ProjectRating).where(ProjectRating.project_id == project_id))
...
async def recalculate_rating(project_id: int, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if old:
        await db.delete(old)
        await db.flush()
    new_rating = calculate_rating(project)
    db.add(new_rating)
    await db.commit()
